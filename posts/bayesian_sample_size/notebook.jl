...
# ╔═╡ e6259178-e233-4805-b4ab-366120e6f845
...
# ╔═╡ 6298bc1a-23c2-11f1-8027-b70e74f22338
md"""
...
# ╔═╡ a791a64b-babe-40b4-aac3-e53e038cc837
md"""
# Overall Bayesian Approach
- Choose the arm $j$ with highest posterior mean.
- Choose $n$ to make the posterior variances small.
- Equivalently, choose $n$ to minimize expected posterior regret ``E[\delta_k - \delta_j]`` over prior predictive. 

```math
\begin{align*}
E[\delta_k - \delta_j] &= E[(\delta_k - E[\delta_k]) + (E[\delta_k] - E[\delta_j])
+ (E[\delta_j] - \delta_j)] \\
&\leq E[(\delta_k - E[\delta_k]) + (E[\delta_j] - \delta_j)] \\
&\leq \text{std}(\delta_k) + \text{std}(\delta_j) \\
\end{align*}
```
"""

# ╔═╡ 8a681cae-fb00-468c-8933-15bfdd3f335a
md"""
...
# ╔═╡ f47b8762-243c-4026-a53f-42ca49be982a
md"""
# A Normal Model
- Control arm with rate $\theta_0 \sim \mathcal{N}(p, \tau_0)$
- Treatment effects $\delta_j \sim \mathcal{N}(0, \tau_1)$
- Observations $y_i \sim \mathcal{N}(\theta_i, \sigma^2)$.
- Treatment arms have rates $\theta_j = \theta_0 + \delta_j$. 
- By CLT, posteriors will resemble those of Beta Bernoulli for high $n$.
- No Monte Carlo integration required!
"""

# ╔═╡ 0def1167-66e5-46f8-80d6-5bba44cd088b
md"""
# Bayesian Linear Regression
Let $w = (\theta_0, \delta)$ so $\theta = Xw$. Parameterizing by precision :

```math
\begin{align*}
w &\sim \mathcal{N}\left(\begin{bmatrix} p \\ 0 \end{bmatrix}, \Lambda\right) \\
w | y &\sim \mathcal{N}(m, L) \\
L &= \Lambda + \frac{n}{\sigma^2}X^TX \\ 
\text{std}(\delta_{j-1}) &= L^{-1}_{jj}
\end{align*}
```
"""

# ╔═╡ b807d9be-77d4-4776-b676-6444ff8d87b6
label(name, val) = "$name: $val";

# ╔═╡ 96bf9377-a9d7-4f3d-b1fb-33b76de79fa9
...
# ╔═╡ 5143f2c4-12d3-476b-b3af-9fb7ef246cdc
md"# Normal Model Results"

# ╔═╡ e1ac85bb-9013-4628-888e-a678c9a8c41c
std_samples(k, c) = find_zero(n-> posterior_std(k, n) - c, (100.0, 10_000.0));

# ╔═╡ f52391e1-bd98-490d-a214-def005baef16
dfz = crossjoin(DataFrame(k=[2,4,6]), DataFrame(c=LinRange(0.015, 0.005, 5)));

# ╔═╡ 61a1216f-478b-4427-a5b3-c3f47b967b0c
...
# ╔═╡ 1b7c11ea-c23e-4c02-8fb6-864b1e2b4170
md"# Total Samples Required"

# ╔═╡ 3b570962-b12c-486b-b95d-b473e6885d61
total_std_samples(k, c) = find_zero(n-> posterior_std(k, n/k) - c, (10.0, 50_000.0));

# ╔═╡ e7f956c7-4f70-46ec-8ac7-9177beff2a6f
draw(data(@transform(dfz,
		:n=total_std_samples.(:k, :c), :k=CategoricalVector(:k))) *
	mapping(:c, :n, color=:k) * visual(Lines),
	scales(Y=(;scale=log10, tickformat=values -> string.(trunc.(Int, values)))),
	figure = (;title="Total Sample Sizes for Posterior Precision"))
...
manifest_format = "2.0"
project_hash = "cbaf99dd7e7ee9bb9069297313b08254c6f0b41a"
...
git-tree-sha1 = "a671a0921eb7bd717bda4013a391110c991d490e"
